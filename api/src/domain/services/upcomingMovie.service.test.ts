...
describe("UpcomingMovieService", () => {
  ...
  describe("calculateDaysUntilRelease", () => {
    it("公開までの残り日数を計算する", () => {
      ...
      const result = service.calculateDaysUntilRelease(movie, today);
      expect(result).toBe(4); // 2024-06-01 から 2024-06-05 まで 4日
    });
  ...
  describe("getBadgeLabel", () => {
    it("残り日数に応じたバッジラベルを返す", () => {
      expect(service.getBadgeLabel(0)).toBe("本日公開");
      expect(service.getBadgeLabel(1)).toBe("明日公開");
      expect(service.getBadgeLabel(2)).toBe("あと2日");
      expect(service.getBadgeLabel(3)).toBe("あと3日");
      expect(service.getBadgeLabel(4)).toBeNull();
      expect(service.getBadgeLabel(null)).toBeNull();
    });
  });
  ...
  describe("getDisplayDate", () => {
    it("表示用日付文字列を返す", () => {
      ...
      const result = service.getDisplayDate(movie);
      expect(result).toBe("6月1日(土)"); // DateUtils.formatReleaseDateToJa の仮定
    });
