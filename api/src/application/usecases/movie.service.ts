...
export class MovieService {
  ...
  async searchMoviesByPersonName(name: string): Promise<Movie[]> {
    // 1. 名前で人物を検索
    const personResponse = await this.tmdbRepository.searchPerson(name);
    ...
    const moviesResponse = await this.tmdbRepository.getDiscoverMovies(params);
